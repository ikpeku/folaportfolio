...
const Footer = ({ reversed = false , isDetail = false, onClick}: { reversed?: boolean , isDetail?: boolean, onClick?: () => void}) => (
  <footer className={`mt-24 pt-12 border-t border-dashed border-[#B1B1B8] flex items-end ${reversed ? "flex-row-reverse" : ""}`}>
...
{isDetail && <div className="flex-1">
  ...
  <button onClick={onClick} className="group mb-6">
            ...
            <p className="font-semibold text-[20px] max-w-xl">Helping healthcare practitioners track their time so they can be appropriately reimbursed by the government </p>
  <img src={Assets.Logo} alt="Folarin Lawal" className="h-8 w-auto mt-28 mb-8" />
    ...
    <div className="flex justify-center">
      <img
        ...
        alt="Footer banner"
        className="w-full max-h-112.75 max-w-sm md:max-w-full rounded-xl object-contain"
      />
    </div>
